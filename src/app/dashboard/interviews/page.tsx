  ...
  return (
    <div className="min-h-screen bg-gray-950 px-6 py-8">
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-gray-400">{interviews.length} interview invitations</p>
      </div>
      ...
      {interviews.length === 0 ? (
        ...
        <div className="space-y-4">
          {interviews.map(interview => (
            <div key={interview.id} className="rounded-xl border border-white/10 bg-gray-900 p-5">
              <div className="mb-3 flex items-start justify-between gap-3">
                <div>
                  <p className="text-sm font-semibold text-white">
                    Interview with {interview.recruiter.companyName || interview.recruiter.name}
                  </p>
                  {interview.notes && (
                    <p className="mt-1 text-xs text-gray-400">{interview.notes}</p>
                  )}
                </div>
              ...
              {interview.confirmedSlot ? (
                <div className="rounded-lg bg-emerald-950/30 border border-emerald-500/20 px-3 py-2 text-sm text-emerald-400">
                  ✓ Confirmed: {new Date(interview.confirmedSlot).toLocaleString()}
                  {interview.meetingLink && (
                    <a href={interview.meetingLink} target="_blank" rel="noopener noreferrer"
                      className="ml-3 underline hover:text-emerald-300">Join Meeting →</a>
                  )}
                </div>
              ) : interview.status === "PENDING" ? (
                <div>
                  <p className="mb-2 text-xs text-gray-400">Select a time slot:</p>
                  <div className="flex flex-wrap gap-2">
                    {interview.proposedSlots.map(slot => (
                      <button key={slot}
                        onClick={() => handleConfirm(interview.id, slot)}
                        disabled={confirming === interview.id}
                        className="rounded-lg border border-emerald-500/30 px-3 py-1.5 text-xs text-emerald-400 hover:bg-emerald-950/30 disabled:opacity-50 transition-colors">
