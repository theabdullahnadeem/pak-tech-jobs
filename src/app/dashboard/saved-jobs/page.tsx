    ...
    <div className="min-h-screen bg-gray-950 px-4 sm:px-6 py-6 sm:py-8">
      ...
      {saved.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-xl border border-white/10 bg-gray-900 py-16 px-4 text-center">
          ...
          <Link href="/jobs" className="rounded-lg bg-emerald-600 px-4 py-2 text-sm font-medium text-white hover:bg-emerald-700 transition-colors">
            Browse Jobs
          </Link>
        </div>
      ) : (
        <div className="space-y-3">
          {saved.map(item => (
            <div key={item.id} className="rounded-xl border border-white/10 bg-gray-900 px-4 sm:px-5 py-4">
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-2 flex flex-wrap gap-1">
                    ...
                    {item.jobPost.skills.length > 4 && (
                      <span className="rounded-full bg-gray-800 px-2 py-0.5 text-xs text-gray-500">+{item.jobPost.skills.length - 4}</span>
                    )}
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 sm:shrink-0">
                  {!item.jobPost.isClosed && (
                    <Link href={`/jobs/${item.jobPost.id}`}
                      className="flex-1 sm:flex-none text-center rounded-lg bg-emerald-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-emerald-700 transition-colors">
                      Apply
                    </Link>
                  )}
                  <button
                    ...
                    className="flex-1 sm:flex-none rounded-lg border border-white/10 px-3 py-1.5 text-xs text-gray-400 hover:bg-white/5 disabled:opacity-50 transition-colors">
